...
# 把每小时的交通参与者数据根据type划分到不同文件里：/type/hour/7hour/type_{type}.json
import json
import os
...
# 划分后文件的输出路径
output_directory="D:/VIScode/chinavis-2023/backend/data/situation/time/type/hour/"


# 创建小时文件夹（如果不存在）
def create_hour_folder(hour):
    folder_name=f"D:/VIScode/chinavis-2023/backend/data/situation/time/type/hour/{hour}hour"
    if not os.path.exists(folder_name):
        os.makedirs(folder_name)
...
for i in range(7, 16):
    ...
    for vehicle in vehicle_data:
        type=vehicle['type']
        file_name=f"{i}hour/type_{type}.json"
        with open(output_directory+file_name,"a") as f:
            json.dump(vehicle,f)
            f.write('\n')
