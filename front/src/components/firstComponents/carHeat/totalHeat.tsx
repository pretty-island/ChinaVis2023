...
interface TotalHeatProps {
    setTypeName: React.Dispatch<React.SetStateAction<string>>;
    typeName: string;
}
const TotalHeat: React.FC<TotalHeatProps> = ({ typeName, setTypeName }) => {
    ...
    useEffect(() => {
        ...
        else if (typeName?.replace("流量","") in typeMapping1) {
                const hourFlow = useData[typeMapping1[typeName?.replace("流量","") ]]
