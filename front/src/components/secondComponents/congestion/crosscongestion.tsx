...
import { getCrossCongestion } from '../../../apis/api';
...
const CrossCongestionParallel: React.FC<CongestionProps> = ({ setTurnName, setSelectedCross, setSelectedMin, selectedHour }) => {
  const chartRef = useRef<HTMLDivElement>(null);
  const [congData, setCongData] = useState<QueData[]>([]);
  // 获取数据
  useEffect(() => {
    getCrossCongestion("/getCrossCongestion").then((res) => {
      const data = res.data;
      setCongData(data);
    });
  }, []);
  ...
  useEffect(() => {
    if (chartRef.current !== null) {
      ...
      const hours = [
        '路口1_左', '路口1_右', '路口1_下', '路口1_上', '路口2_左', '路口2_右', '路口2_下', '路口2_上',
        '路口3_左', '路口3_右', '路口3_下', '路口3_上', '路口4_左', '路口4_右', '路口4_下', '路口4_上', '路口5_左',
        '路口5_右', '路口5_上', '路口6_左', '路口6_右', '路口6_上',
        '路口7_左', '路口7_右', '路口7_上', '路口8_左', '路口8_右', '路口8_上',
      ];
      // prettier-ignore
      const days = [
        '00', '05', '10',
      ...
      const option: EChartOption = {
        ...
        graphic: [
          {
            ...
            top: '8',
            style: {
              text: '时间：' + selectedHour + "     " + "路口各方向拥堵情况",
              fill: '#fFF',
              fontSize: 13,
              // fontWeight: 'bold'
            }
        ...
        visualMap: [
          ...
          },
          {
            ...
            // inRange: {
            //   color: ['green','green','#d5c173','#d5c173','#d5c173','#d5c173','#852D30','#852D30','#852D30','#852D30','#852D30','#852D30','#852D30','#852D30','#852D30','#852D30','#852D30','#852D30','#852D30']
      ...
      mychart.on('click', 'series', (params) => {
        ...
        // console.log(min);        
        setSelectedMin(min)
        setSelectedCross(params.name.split("_")[0].replace("路口", ''))
        setTurnName(params.name.split("_")[1])
      })
    }
  }, [congData, selectedHour, setTurnName, setSelectedMin, setSelectedCross])
  return (
    <div ref={chartRef} style={{ width: "100%", height: "95%" }}></div>
  )
}
export default CrossCongestionParallel;